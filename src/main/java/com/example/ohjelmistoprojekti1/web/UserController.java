...
public class UserController {
	...
	public User user(@RequestBody Map<String, Object> userinfo) {
		...
		try {
			...
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Käyttäjä on jo olemassa", e);
	...
	@ResponseStatus(value = HttpStatus.NO_CONTENT)
	public User editUser(@PathVariable Long userid, Map<String, String> props) {
		try {
			User u = urepo.findById(userid).get();
			props.keySet().stream().map((key) -> key.trim().toLowerCase());
			if (props.keySet().contains("firstname")) {
				u.setFirstname(props.get("firstname"));
			}
			if (props.keySet().contains("lastname")) {
				u.setLastname(props.get("lastname"));
			}
